...
mcp.tool(
  ...
  async ({ group_by, application_type, marketing_status, year_from, year_to, limit }) => {
    ...
    let groupCol;
    switch (group_by) {
      case "sponsor": groupCol = "a.sponsor_name"; break;
      case "pharm_class": groupCol = "a.pharm_class"; break;
      case "application_type": groupCol = "a.application_type"; break;
      case "marketing_status":
        ...
        const msRes = await pool.query(`
          ...
          LIMIT ${safeLimit}
        `, params);
        const msText = msRes.rows.map(r => `${r.group_key || "(empty)"}: ${r.count}`).join("\n");
        return { content: [{ type: "text", text: `Marketing status distribution:\n\n${msText}` }] };
      case "year":
        const yrRes = await pool.query(`
          ...
          ${where.replace(/\ba\./g, "a.")}
          AND s.submission_status_date IS NOT NULL
          GROUP BY group_key
          ORDER BY group_key DESC
          LIMIT ${safeLimit}
        `, params);
        const yrText = yrRes.rows.map(r => `${r.group_key}: ${r.count} applications`).join("\n");
        return { content: [{ type: "text", text: `Applications by year:\n\n${yrText}` }] };
      default:
        return { content: [{ type: "text", text: "Invalid group_by value." }] };
    }
...
// Tool 4: Run custom SQL (for power users / advanced questions)
mcp.tool(
  "run_query",
  ...
  async ({ sql }) => {
    ...
    try {
      const res = await pool.query(sql + (sql.includes("LIMIT") ? "" : " LIMIT 50"));
      if (res.rows.length === 0) {
